...
@section("content")
<div class="container">
    <form method="post" action="/appointments/update/{{ $model->AppointmentId }}" id="form">
        @csrf
        <div class="row gy-3">
            ...
            <div class="col-md-12 col-lg-6 col-xxl-4">
                <select class="form-select" name="PetId">
                    @foreach($pets as $pet)
                        <option value="{{ $pet->PetId }}" {{ $pet->PetId == $model->PetId ? 'selected' : '' }}>{{ $pet->PetName }}</option>
                    @endforeach
                </select>
            ...
            <div class="col-md-12 col-lg-6 col-xxl-4">
                <select class="form-select" name="VetId">
                    @foreach($vets as $vet)
                        <option value="{{ $vet->VetId }}" {{ $vet->VetId == $model->VetId ? 'selected' : '' }}>{{ $vet->VetName }} {{ $vet->VetSurname }}</option>
                    @endforeach
                </select>
